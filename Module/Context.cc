...
void Context::context(struct TNode* n)
{
	...
	if(n != 0)
	{
		...
		case PROGRAM                    :
			{ // Kontextprüfung fast fertig, Infomationen ausgeben
				b.push(1);
				context(n->n1);
				cout<<"\nNamensliste:\n"; nl.out();
				cout<<"\nFunktionsliste:\n"; fl.out();
		...
		case VAR_DECL_ST_2              :
			{
				decl++; // Deklarationen mitzählen, um sie von denen des hauptblocks zu unterscheiden
		...
		case CHAR_CONSTANT              :
			{
				if(minus) cl.insert(-(char)n->n1, ++cc);  else { cl.insert((char)n->n1, ++cc); lastconst = (char)n->n1;  }
				break;
			}
		case INT_CONSTANT               :
			{
				if(minus) cl.insert(-(int)n->n1, ++cc); else { cl.insert((int)n->n1, ++cc); lastconst = (int)n->n1;  }
				break;
			}
		case FLOAT_CONSTANT             :
			{
				if(minus) cl.insert(-*(double*)n->n1, ++cc); else cl.insert(*(double*)n->n1, ++cc);
				break;
			}
		...
		case FUNCTION_CALL_1            :
			{
				call=true; context(n->n1);call=false;
		...
		case FUNCTION_CALL_2            :
			{
				call=true; ins(0); context(n->n1); context(n->n2); context(n->n3);call=false;
				{
					char* c = s.top();
